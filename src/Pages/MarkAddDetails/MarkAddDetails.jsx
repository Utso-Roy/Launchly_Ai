  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 py-10 px-4 md:px-12 lg:px-32">
      <div className="max-w-3xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 md:p-10">
        <button onClick={() => navigate(-1)} className="mb-6 flex items-center text-[#21BEDA] font-semibold">
          <FaArrowLeft className="mr-2" /> Back to Products
        </button>

        <img src={product?.data?.image} alt={product?.data?.name} className="w-full h-[300px] object-cover rounded mb-6" />

        <h2 className="text-3xl font-bold text-[#21BEDA] mb-2">{product?.data?.name}</h2>

        <p className="text-gray-700 dark:text-gray-300 mb-4">{product?.data?.description || "No description available."}</p>

        {product?.data?.link && (
          <a href={product.data.link} target="_blank" rel="noreferrer" className="text-blue-600 underline mb-4 block">
            Visit Product Site
          </a>
        )}
        ...
        <div className="flex items-center gap-4 mb-6">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded ${
              isOwner || hasVoted ? "bg-gray-400 cursor-not-allowed" : "bg-[#21BEDA] hover:bg-[#1ca6c0]"
            } text-white`}
          >

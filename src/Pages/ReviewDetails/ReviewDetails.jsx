...
const ReviewDetails = () => {
  ...
  if (!product) return <div className="text-center text-white">Loading Product Details...</div>;
  ...
  return (
    <div className="min-h-screen  my-4 bg-gray-100 text-white p-6">
      ...
      <div className="max-w-xl mx-auto  bg-gray-50 rounded-xl shadow-lg overflow-hidden p-6 space-y-6">
        {/* Product Image */}
        <img src={product?.data?.image} alt={product.name} className="w-full max-h-[300px] object-cover rounded-lg" />

        {/* Product Details */}
        <div className="space-y-2">
          <h2 className="text-2xl font-bold text-[#21BEDA]">{product?.data?.name}</h2>
          <p className="text-[#201571]">{product?.data?.description}</p>

          {/* Tags */}
          <div className="flex flex-wrap gap-2 mt-3">
            {product?.data.tags?.map((tag, i) => (
              <span key={i} className="px-3 py-1 bg-[#21BEDA] text-white rounded-full text-sm">#{tag}</span>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between mt-4">
            <span className={`px-3 py-1 rounded-full text-sm font-medium ${
              product.status === "Pending" ? "bg-yellow-600" : product.status === "Accepted" ? "bg-green-600" : "bg-red-600"
            }`}>
              Status: {product.status}
            </span>
            <span className="text-sm text-gray-400">Upvotes: {product.upvotes}</span>
          </div>
        </div>
